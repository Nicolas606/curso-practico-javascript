...
function encntrarAltura(lado, base){
  const baseIsosleses = base / 2;
  const resultado = (lado * lado) - (baseIsosleses * baseIsosleses)
  return Math.sqrt(resultado)

}

function calcularAlturaTriangulo(){
  const lados = document.getElementById('InputIsoceles');
  const ladoBase = document.getElementById('InputIsocelesBase');
  const valueLados = parseInt(lados.value)
  const valueBase = parseInt(ladoBase.value)
  const altura = encntrarAltura(valueLados, valueBase);
  alert(altura)
}
